...
class Xoo_Wl_Admin_Settings{
	...
	public function __construct(){
		$this->capability = isset( xoo_wl_helper()->admin->capability ) ? xoo_wl_helper()->admin->capability : 'manage_woocommerce';
		$this->hooks();	
	}
	...
	public function display_preview_template_form( $tab_id, $tab_data ){
		if( $tab_id === 'email' || $tab_id === 'email-style' ){
			echo $this->get_preview_template_form();
		}
	...
	public function preview_email(){
		if( isset( $_GET['page'] ) && $_GET['page'] === 'waitlist-woocommerce-settings' && isset( $_GET['preview'] ) ){
			...
			$type = isset( $_GET['type'] ) ? $_GET['type'] : 'backinstock';
	...
	public function view_waitlist_page(){
		...
		if( isset( $_GET['product'] ) && $_GET['product'] ){
			...
			$args['count'] 			= xoo_wl_db()->get_waitlisted_count( $_GET['product'] );
			$args['rows'] 			= xoo_wl_db()->get_waitlist_rows_by_product( $_GET['product'] );
			$args['product_id'] 	= $_GET['product'];
