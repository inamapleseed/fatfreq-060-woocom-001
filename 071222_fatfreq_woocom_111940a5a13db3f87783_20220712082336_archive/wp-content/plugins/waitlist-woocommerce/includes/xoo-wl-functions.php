...
function xoo_wl_is_product_out_of_stock( $product_id ){
	...
	if( $product ){
		return !$product->is_in_stock() || ( xoo_wl_helper()->get_general_option( 'm-en-bod' ) === "yes" &&  $product->get_stock_status() === "onbackorder" && ( !$product->get_manage_stock() || $product->get_stock_quantity() <= 0 ) );
	}
