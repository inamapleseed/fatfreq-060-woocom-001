...
if ( ! class_exists( 'WC_Admin_Duplicate_vpc_configuration' ) ) :

    /**
     * WC_Admin_Duplicate_wpc-template Class
     */
    class WC_Admin_Duplicate_vpc_configuration {
	...
	public function dupe_link( $actions, $post ) {
	    if ( ! current_user_can( apply_filters( 'woocommerce_duplicate_vpc-config_capability', 'manage_woocommerce' ) ) ) {
		return $actions;
	    }

	    if ( $post->post_type != 'vpc-config' ) {
		return $actions;
	    }
	...
	public function dupe_button() {
	    ...
	    if ( isset( $_GET[ 'post' ] ) ) {
		$notifyUrl = wp_nonce_url( admin_url( "edit.php?post_type=vpc-config&action=duplicate_vpc-config&post=" . absint( $_GET[ 'post' ] ) ), 'woocommerce-duplicate-vpc-config_' . $_GET[ 'post' ] );
		?>
		<div id="duplicate-action"><a class="submitduplicate duplication" href="<?php echo esc_url( $notifyUrl ); ?>"><?php _e( 'Copy to a new draft', 'vpc' ); ?></a></div>
		<?php
	    }
	}
	...
	public function duplicate_vpc_config_action() {
		...
		wp_die( __( 'No configuration to duplicate has been supplied!', 'vpc' ) );
	    }
		...
		do_action( 'woocommerce_duplicate_vpc-config', $new_id, $post );

		// Redirect to the edit screen for the new draft page
		wp_redirect( admin_url( 'post.php?action=edit&post=' . $new_id ) );
		exit;
	    } else {
		wp_die( __( 'Configuration creation failed, could not find original configuration:', 'vpc' ) . ' ' . $id );
	    }
	}
...
return new WC_Admin_Duplicate_vpc_configuration();
