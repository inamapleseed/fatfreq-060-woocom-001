...
class VPC_Config {
	...
	if ( class_exists( 'Ofb' ) ) {
	    global $wpdb;
	    $ofb		 = array( 'None' => 'None' );
	    $resultats	 = $wpdb->get_results( "SELECT ID,post_title From {$wpdb->prefix}posts Where post_type = 'ofb' And post_status = 'publish'", OBJECT_K );
	    if ( $resultats ) {
	...
	if ( $product_link == 'Yes' && class_exists( 'Woocommerce' ) ) {
	...
	<div id="vpc-preview">
		...
		if ( isset( $component[ 'options' ] ) && is_array( $component[ 'options' ] ) ) {
			...
			if ( $is_default && ! empty( $option_image ) ) {
			    ...
			    // echo "<img data-component='" . $component[ "component_id" ] . "' src='$option_image'>";
			    break;
			}
		    }
		}
	...
	<script>
	    var vpc_preview_images = <?php echo json_encode( $preview_images ); ?>;
	    var vpc_products = [<?php echo implode( ',', $products ); ?>];
	</script>
	<?php
 ...
 $title
    ...
    function add_rules_columns( $defaults ) {
	$defaults[ 'vpc_manage_rules' ] = __( 'Manage rules', 'vpc' );
	return $defaults;
    }

    function get_rules_columns_values( $column_name, $id ) {

	if ( $column_name === 'vpc_manage_rules' ) {
	    $url = admin_url( "edit.php?post_type=vpc-config&page=manage-conditional-rules&config-id=$id" );
	    echo "<a class='button button-primary button-large ' href='$url'>" . __( 'Manage conditional rules', 'vpc' ) . '</a>';
	}
    }
