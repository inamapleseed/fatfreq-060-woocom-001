...
class VPC_Updating_Manager {
	...
	function __construct( $current_version, $update_path, $plugin_slug ) {
		...
		add_action( 'in_plugin_update_message-' . VPC_MAIN_FILE, array( &$this, 'addUpgradeMessageLink' ) );
	}
	...
	public function addUpgradeMessageLink() {
		...
		if ( isset( $options['purchase-code'] ) && $options['purchase-code'] != '' ) {
				echo '<a href="' . wp_nonce_url( admin_url( 'update.php?action=upgrade-plugin&plugin=' . VPC_MAIN_FILE ), 'upgrade-plugin_' . VPC_MAIN_FILE ) . '">' . __( 'Update Visual Products Configurator now.', 'vpc' ) . '</a>';
		} else {
			$url = admin_url( 'admin.php?page=vpc-manage-settings' );
			echo ' <a href="' . $url . '">' . __( 'You need a licensee key to update this plugin. This is the license key you received after your purchase.<a href="https://configuratorsuiteforwp.com/guide/how-to-add-my-license/" target="_blank">Learn more</a>.', 'vpc' ) . '</a>';
		}
	}
}
