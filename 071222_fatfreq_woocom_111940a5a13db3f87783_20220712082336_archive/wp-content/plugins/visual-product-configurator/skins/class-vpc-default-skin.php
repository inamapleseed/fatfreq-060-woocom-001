...
class VPC_Default_Skin {
	...
	vpc_skins_enqueue_styles_scripts( 'VPC_Default_Skin' );
	ob_start();
	...
	<div id="vpc-container" class="o-wrap <?php echo $skin_name; ?>" data-curr="<?php echo ( class_exists( 'Woocommerce' ) ) ? get_woocommerce_currency_symbol() : ''; ?>">
		...
		if ( class_exists( 'Vpc_Sfla' ) ) {
		    $Save_class = new Vpc_Sfla_Public( false, VPC_SFLA_VERSION );
		    echo $Save_class->get_all_configs( $config, $product_id );
		}
		?>
	    </div>
	    <div id="debug"></div>
    ...
    public function get_components_block( $component, $options_style, $config, $config_to_load = array() ) {
	...
	if ( $component[ 'cimage' ] ) {
	    $c_img_url = o_get_proper_image_url( $component[ 'cimage' ] );
	}
	if ( isset( $c_img_url ) ) {
	    $image_resize	 = aq_resize( $c_img_url, 60, 60, true );
	...
	<div id = '<?php echo $component_id; ?>' class="vpc-component" <?php echo $components_attributes_string; ?>>
		...
		if ( ! is_array( $options ) || empty( $options ) ) {
		    _e( 'No option detected for the component. You need at least one option per component for the configuration to work properly.', 'vpc' );
		} else {
