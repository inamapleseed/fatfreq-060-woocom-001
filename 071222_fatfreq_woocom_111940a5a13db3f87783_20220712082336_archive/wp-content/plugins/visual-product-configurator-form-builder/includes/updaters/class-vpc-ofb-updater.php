...
class Vpc_Ofb_Updater {

    protected $version_url = 'https://static.configuratorsuiteforwp.com/vpc-ofb-updater.xml';

    public $title = VPC_OFB_NAME;
     ...
     *
     * @param Vpc_Ofb_Updating_Manager $updater
     */
    public function setUpdateManager(Vpc_Ofb_Updating_Manager $updater) {
     ...
     *
     * @return Vpc_Ofb_Updating_Manager
     */
    ...
    public function upgradeFilter($reply, $package, $updater) {
        ...
        if ( (isset($updater->skin->plugin) && $updater->skin->plugin === VPC_OFB_MAIN_FILE) ||
                (isset($updater->skin->plugin_info) && htmlspecialchars_decode($updater->skin->plugin_info['Name']) === $this->title)
        ) {
            $updater->strings['download_from_servers'] = __('Downloading package from ORION servers...', 'vpc-ofb');
            $updater->skin->feedback('download_from_servers');
            $package_filename = 'visual-product-configurator-form-builder.zip';
            $res = $updater->fs_connect(array(WP_CONTENT_DIR));

            if (!$res) {
                return new WP_Error('no_credentials', __("Error! Can't connect to filesystem", 'ofb'));
            }
            $options = get_option('vpc-options');
            if (isset($options['purchase-code-form-builder-add-on']) && !empty($options['purchase-code-form-builder-add-on'])) {
                $license_key = $options['purchase-code-form-builder-add-on'];
            } else {
                return new WP_Error('no_credentials', __('A license key is required to receive automatic updates (<a href=“https://configuratorsuiteforwp.com/guide/how-to-add-my-license/” target=“_blank”>Learn more</a>). Please visit <a href=“https://configuratorsuiteforwp.com/my-account/orders/“ target=“blank”>your account</a> to retrieve it.' . admin_url( 'edit.php?post_type=vpc-config&page=vpc-manage-settings&section=vpc-ofb-container' ) . '' . '" target="_blank">Settings</a> to activate your Visual Products Configurator Form Builder Add-On.', 'vpc-ofb'));
            }

            $args = array('timeout' => 600);
            $site_url = get_site_url();
            $plugin_name = VPC_OFB_NAME;
            ...
            if (!is_wp_error($response) ) {
                if (isset($response["body"]) && intval($response["body"]) == 200) {
                    ...
                    if (is_array(json_decode($answer, true))) {
                        ...
                        return new WP_Error('no_file', __('Error! No file found. Please contact the plugin owners.', 'ofb'));;
                    }
                    ...
                    return new WP_Error('no_credentials', __('Error on unzipping package', 'ofb'));
                } else {
                    return new WP_Error('network_error', __('Wrong license key provided. Please add the correct one and try again.', 'ofb'));
                }
            } else {
    ...
    protected function downloadUrl( $title ) {
            ...
            $purchase_code = "";
            $options = get_option('vpc-options');
            if (isset($options['purchase-code-form-builder-add-on']) && $options['purchase-code-form-builder-add-on'] !== "") {
                $purchase_code = $options['purchase-code-form-builder-add-on'];
            }
